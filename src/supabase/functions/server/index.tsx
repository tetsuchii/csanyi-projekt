...
// Utility function to sanitize filenames/paths for storage
const sanitizePath = (path: string): string => {
  ...
  const sanitizedParts = parts.map((part, index) => {
    ...
    const name = lastDotIndex !== -1 ? part.substring(0, lastDotIndex) : part;
    const ext = lastDotIndex !== -1 ? part.substring(lastDotIndex) : '';
...
app.post("/make-server-5be515e6/storage/upload-url", async (c) => {
    try {
        ...
        return c.json({ url: data.signedUrl, token: data.token, path: sanitizedPath });
    } catch (err) {
...
app.post("/make-server-5be515e6/storage/get-url", async (c) => {
    try {
        ...
        // Try sanitized path first (for new files)
        const sanitizedPath = sanitizePath(path);
        let { data, error } = await supabase.storage
            .from(BUCKET_NAME)
            .createSignedUrl(sanitizedPath, 7200);

        // If sanitized path fails and it's different from original, try original path (for legacy files)
        if (error && sanitizedPath !== path) {
            console.log('Sanitized path failed, trying original path for legacy file:', path);
        ...
        if (error) {
            console.log('Storage object not found:', path, '(sanitized:', sanitizedPath + ')');
...
app.post("/make-server-5be515e6/share", async (c) => {
    try {
        ...
        await kv.set(key, { userId: user.id, projectId });
